export default {
  id: "picture",
  list: [
    {
      id: "animals",
      label: "Animals-జంతువులు-Jantuvulu",
      text: `cat | పిల్లి | Pilli
dog | కుక్క | Kukka
goat| మేక | Meka
cow| 	ఆవు| avu
donkey| 	గాడిద | Gadida
horse| 	గుర్రం | gurram
pig| పంది | pandi
elephant| ఏనుగు | enugu
lion | సింహం | sinham
monkey| కోతి | koti

hippo| నీటి ఏనుగ | niti enuga
rhino| ఖడ్గమృగం | khadgamrgam
fox| ఫాక్స్ | Phaks
crocodile| మొసలి | Mosali
zebra| 	చారల గుర్రము | Carala gurramu
bear| బేర్ | Ber
camel| 	ఒంటె | Onte
shark|షార్క్| Shark
whale| తిమింగలం|Timingalam
dolphin| డాల్ఫిన్|Dolphin

tiger|పులి | puli
sheep| 	గొర్రె | gorre
fish| చేప| Cepa
frog| కప్ప |Kappa
crab| పీత |Pita
snake| పాము | pamu
bull | బుల్ | Bull
tortoise| తాబేలు |tabelu
rat| ఎలుక | eluka
ant| చీమ| Cima

bat| బ్యాట్| Byat
buffalo|బఫెలో | Baphelo
butterfly| సీతాకోకచిలుక| Sitakokaciluka
cheetah| చిరుత|Ciruta
bee | తేనెటీగ|Tenetiga
goldfish | గోల్డ్ ఫిష్|Goldfish
spider | సాలీడు|Salidu
panda | పాండా|Panda
giraffe| 	జిరాఫీ | Jiraphi
kangaroo| 	ఒక సాదు జంతువు | oka sadu jantuvu`,
    },
    {
      id: "birds",
      label: "Birds-పక్షులు-Paksulu",
      text: `crow|కాకి | Kaki
dove|పావురం| Pavuram
duck|	డక్ |Duck
hen|	కోడి | Kodi
chick| పౌల్ట్రీ |Paultri
parrot|చిలుక |Ciluka
peacock| నెమలి | Nemali
penguin|పెంగ్విన్|Penguin
sparrow|పిచ్చుక | Piccuka
crane| 	కొంగ |Konga

eagle| 	డేగ | dega
flamingo| రాజహంస|Rājahansa
kingfisher| చల్లముద్దపక్షి| Callamuddapaksi
myna| మైనా|Myna
ostrich| 	ఉష్ట్రపక్షి |Ustrapaksi
owl| గుడ్లగూబ |Gudlaguba
rooster| రూస్టర్ |Rooster
vulture| రాబందు |Rabandu
woodpecker| 	వడ్రంగిపిట్ట |Vadrangipitta
koel |కోయెల్|  Koel`,
    },
  ],
};
