export default {
  id: 'picture-5',
  label: 'Action and Sports',
  list: [
    {
      id: 'action',
      label: 'చర్యలు|Caryalu|Actions',
      text: `drink| త్రాగండి|Tragandi
      eat| తినండి|Tinandi
      bend|బెండ్|Bend
      climb | ఎక్కడం|Ekkadam
      jump | ఎగిరి దుముకు|Egiri dumuku
      kneel | మోకాలి|Mokali
      pull | Pull
      push | పుష్|Pus
      sing | పాడండి|Padandi
      dance | నృత్యం|Nrtyam
      running |నడుస్తోంది|Nadustondi
      walking | నడవడం|Nadavadam
      sit | కూర్చోండి|Kurcondi
      party | పార్టీ|Party
      skipping | దాటవేయడం|Dataveyadam
      catch | క్యాచ్|Catch
      pass | పాస్|Pass
      swim | ఈత |Ita
      swing | స్వింగ్|Swing
      throw | త్రో|Throw
      carry | తీసుకువెళ్ళండి|Tisukuvellandi
      lifting | Lift
      mopping | మోపింగ్|Moping
      riding |రైడ్| Ride
      sweep | స్వీప్ చేయండి|Svip ceyandi
      cough | దగ్గు|Daggu
      sick | అనారోగ్యం|Anarogyam
      sneeze | తుమ్ము|Tummu
      yawn | ఆవలించు|avalincu
      sleep | నిద్ర|Nidra`
    },
    {
      id: 'sports',
      label: 'క్రీడలు|Krīḍalu|Sports',
      text: `bat2 | బ్యాట్|Byat
      racket | Racket
      seesaw | Seesaw
      slide | స్లయిడ్|Slayid
      carrom | క్యారమ్|Carrom
      chess | చదరంగం|Cadaraṅgaṁ
      cricket2 | క్రికెట్|Cricket
      tennis | టెన్నిస్|Tennis
      football | ఫుట్బాల్|Football
      hockey | హాకీ|Hockey
      kabaddi | కబడ్డీ|Kabaddi
      archery | విలువిద్య|Viluvidya
      basketball |బాస్కెట్‌బాల్| Basket ball
      boxing | బాక్సింగ్|Boxing
      table-tennis |టేబుల్ టెన్నిస్| table-tennis
      golf | గోల్ఫ్|Golf
      wrestling |రెజ్లింగ్|Rejling
      karate | కరాటే|Karate
      surfing | సర్ఫింగ్|Surfing
      jockey | గుర్రపు పందెం|Gurrapu pandeṁ
      track | నడుస్తున్న ట్రాక్|Nadustunna trak
     bandage | కట్టు|Kaṭṭu
      danger | ప్రమాదం|Pramadam`
    }
  ]
};
